...
# Prompt: Please write skeleton code for the Groups concept. For now all the actions and queries should just return some sort of not implemented error.
# response:

An implementation of the `Groups` concept as a TypeScript class.
...
# Prompt: Looks good. Please implement the create group action
# response:

An implementation of the `createGroup` action for the `Groups` concept.
...
# Prompt: On this line: return { error: `An unexpected error occurred while creating group: ${e.message}` }; e is of type unknown
# response:
...
To solve this, we can perform a type check to ensure `e` is an instance of `Error` before accessing the `message` property. This ensures type safety and resolves the compilation error.
...
# Prompt: Cool, now please implement the inviteMember action
# response:

Excellent. Here is the implementation for the `inviteMember` action.
...
# Prompt: not implement the acceptInvitation and declineInvitation actions
# response:

Of course. Here are the implementations for the `acceptInvitation` and `declineInvitation` actions.
...
# Prompt: Can you write the queries?
# response:

Of course. Here is the implementation for the two queries, `_getGroupDetails` and `_listGroupsForUser`.
...
# Prompt: Can you try writing the last three actions. These should be editGroupName, leaveGroup, and deleteGroup.
# response:

Of course. Here is the final implementation of the `Groups` concept, with the `editGroupName`, `leaveGroup`, and `deleteGroup` actions completed.
  ...
  async deleteGroup({ group }: { group: Group }): Promise<Empty | { error: string }> {
    try {
      ...
      if (groupDoc.members.length > 0) {
        return { error: "Cannot delete group with members." };
      }

      if (groupDoc.invitedMembers.length > 0) {
        return { error: "Cannot delete group with pending invitations." };
      }

      const result = await this.groups.deleteOne({ _id: group });

      if (!result.acknowledged || result.deletedCount === 0) {
        return { error: "Database operation failed: could not delete group." };
      }
